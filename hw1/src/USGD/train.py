...
## sGD 
shufmap = np.arange(len(train_x))
for itr in range(N_iter):
	np.random.shuffle(shufmap)
	sfmaper = np.array([shufmap[BatchSz*i:BatchSz*(i+1)] for i in range(int(len(shufmap)/BatchSz))])
	for smp_idx in sfmaper:
