...
from rnn import *	
if (len(sys.argv) < 1):
	print (".py <.cfg> <dict_path> <data_lbl> <is resume semi>")
	exit(1)
...
semi_rate   = float(pars.val['semi_rate'])
...
sm_data_path = os.path.join(model_dir,"TransLabel.dat")
...
label2, raw2 = Read_label(sm_data_path,tk_dict)
Nsemi = int(len(raw2)*semi_rate)
raw = np.concatenate((raw,raw2[:Nsemi]),axis=0)
label = np.concatenate((label,label2[:Nsemi]),axis=0)
del label2 
del raw2
...
if isresume :
	print ("Resume")
	sys.stdout.flush()
	model.load_state_dict(torch.load(os.path.join(model_dir,'rnn.sm.model')))
else:
...
model.cpu()
torch.save(model.state_dict(),os.path.join(model_dir,'rnn.sm.model'))
